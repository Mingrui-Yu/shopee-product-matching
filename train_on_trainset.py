...
def read_dataset():
    ...
    df['matches_num'] = df['matches'].apply(lambda x: x.count('train_')).values

    grouped = df[df['matches_num']>2].groupby('label_group').size()
    # print(grouped)
    ...
    df['split'] = df['label_group'].map(group_to_split)# 这一步之后，matches_num=2的split列都会变成NaN
    df['is_test'] = df['split'] == 1

    # print(df.head(15))

    # training的matches需要全部是训练集里面的图
    df_training = df[df['is_test']==False].drop('matches',axis=1)
    tmp = df_training.groupby(['label_group'])['posting_id'].unique().to_dict()
    df_training['matches'] = df_training['label_group'].map(tmp)
    df_training['matches'] = df_training['matches'].apply(lambda x: ' '.join(x))
    df_training = df_training.reset_index()
    # print(df_training.head(15))

    # testing的matches需要全部是测试集里面的图
    df_testing = df[df['is_test']==True].drop('matches',axis=1)
    tmp = df_testing.groupby(['label_group'])['posting_id'].unique().to_dict()
    df_testing['matches'] = df_testing['label_group'].apply(lambda x: tmp.setdefault(x,[]))
    df_testing['matches'] = df_testing['matches'].apply(lambda x: ' '.join(x))
    df_testing = df_testing.reset_index()

    # df.to_csv("./temp.csv", index=False)

    # print("!!!!!!!!!!!!!!!", image_paths[11])
    training_image_paths = './shopee-product-matching/train_images/' + df_training['image']
    # print("!!!!!!!!!!!!!!!", training_image_paths[11])
    testing_image_paths = './shopee-product-matching/train_images/' + df_testing['image']

    df_training_cu = cudf.DataFrame(df_training)

    return df_training, df_training_cu, training_image_paths
